...
const MOCK_COURSES: Course[] = Array.from({ length: 24 }).map((_, i) => ({
  ...
  totalLessons: Math.floor(Math.random() * 60 + 10),
  totalSections: Math.floor(Math.random() * 8 + 3),
  totalEnrollments: Math.floor(Math.random() * 5000 + 100),
  ...
  isFeatured: i < 4,
  status: 'PUBLISHED',
  teacher: { id: `t${i}`, firebaseUid: '', email: `teacher${i}@example.com`, firstName: 'John', lastName: 'Doe', displayName: 'John Doe', avatarUrl: null, role: 'TEACHER', phoneNumber: null, bio: null, isActive: true, isEmailVerified: true, lastLoginAt: null, createdAt: '', updatedAt: '' },
